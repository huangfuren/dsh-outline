...
export function apply(ctx, config = {}) {
    ...
    const makeClient = () => {
        ...
        if (!baseUrl || !apiToken) {
            throw new Error('dsh-outline 未配置：需要 baseUrl 与 apiToken（可在 设置 → 插件 → 插件配置 填写，或环境变量 OUTLINE_BASE_URL / OUTLINE_API_TOKEN）。配置方法见插件 README。');
        }
