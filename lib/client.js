...
export class OutlineClient {
    ...
    static MAX_PAGE_SIZE = OUTLINE_MAX_PAGE_SIZE;
    ...
    assertAllowedUrl(baseUrl) {
        ...
        if (isLocal || isPrivate)
            return;
        throw new Error(`dsh-outline 拒绝非 HTTPS 地址：${baseUrl}。为避免 Token 明文传输，公网地址必须使用 https://（localhost、::1、127.0.0.1、内网私有地址除外）。`);
    }
     ...
     */
    async fetchAllPages(fetchPage, pageSize) {
    ...
    async searchDocuments(query, limit, collectionId, filters, offset = 0) {
        ...
        const mapped = data.map((item) => {
            ...
            const createdBy = (document.createdBy ?? record.createdBy);
    ...
    async listCollections(force = false) {
        ...
        const raw = await this.fetchAllPages(async (offset, pageSize) => {
    ...
    async listChildDocuments(parentDocumentId, pageSize = 100) {
        ...
        const docs = await this.fetchAllPages(async (offset, size) => {
     ...
     * 超长文本截断后追加 …标记。
     */
    async getDocumentExcerpt(id, maxChars = 800) {
     ...
     */
    async searchWithExcerpts(query, limit, collectionId, filters, excerptChars = 800) {
        const result = await this.searchDocuments(query, limit, collectionId, filters);
        if (result.hits.length === 0) {
            return { ...result, excerpts: [] };
        }
        const excerpts = await mapWithConcurrency(result.hits, OutlineClient.MAX_CONCURRENCY, (hit) => this.getDocumentExcerpt(hit.id, excerptChars));
        return { ...result, excerpts };
    }
}
